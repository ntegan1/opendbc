...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, experimental_long, docs) -> structs.CarParams:
    ...
    ret.enableDsu = len(found_ecus) > 0 and Ecu.dsu not in found_ecus and candidate not in (NO_DSU_CAR | UNSUPPORTED_DSU_CAR) \
                                        and not bool(ret.flags & ToyotaFlags.SMART_DSU)
    ...
    if ret.flags & ToyotaFlags.SNG_WITHOUT_DSU:
      stop_and_go = stop_and_go or (ret.enableDsu and not docs) or bool(ret.flags & ToyotaFlags.SMART_DSU.value)
    ...
    if candidate in (RADAR_ACC_CAR | NO_DSU_CAR):
      ret.experimentalLongitudinalAvailable = candidate in RADAR_ACC_CAR or bool(ret.flags & ToyotaFlags.SMART_DSU)

      # Disabling radar is only supported on TSS2 radar-ACC cars
      if experimental_long and candidate in RADAR_ACC_CAR and not bool(ret.flags & ToyotaFlags.SMART_DSU):
    ...
    if ret.flags & ToyotaFlags.SECOC.value:
      ...
      ret.openpilotLongitudinalControl = ret.enableDsu or \
        ...
        bool(ret.flags & ToyotaFlags.SMART_DSU)
